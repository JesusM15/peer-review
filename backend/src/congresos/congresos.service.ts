...
export class CongresosService {
  ...
  async findOne(id: string): Promise<Congreso> {
    const congreso = await this.congresoRepo.findOne({ where: { id }, relations: ['tags'] });
    if (!congreso) throw new NotFoundException('Congreso no encontrado');
    return congreso;
  }

  async updateCongreso(id: string, nombre?: string, descripcion?: string, tags?: string[]): Promise<Congreso> {
   ...
   */
  async assignRevisorToTag(userId: string, tagId: string, congresoId: string): Promise<RevisorTag> {
    ...
    if (!membership) throw new BadRequestException('El usuario no es Revisor en este congreso');
    ...
    if (existing) throw new BadRequestException('El revisor ya tiene esta etiqueta asignada');
   ...
   */
  async getRevisorTags(userId: string, congresoId: string): Promise<RevisorTag[]> {
  ...
  async removeRevisorTag(revisorTagId: string): Promise<void> {
    const revisorTag = await this.revisorTagRepo.findOne({ where: { id: revisorTagId } });
    if (!revisorTag) throw new NotFoundException('Asignación de etiqueta no encontrada');
   ...
   */
  async assignCongresoTag(congresoId: string, tagId: string): Promise<CongresoTag> {
    const congreso = await this.congresoRepo.findOne({ where: { id: congresoId } });
    if (!congreso) throw new NotFoundException('Congreso no encontrado');
    ...
    if (existing) throw new BadRequestException('El congreso ya tiene esta etiqueta asignada');
  ...
  async removeCongresoTag(congresoTagId: string): Promise<void> {
    const congresoTag = await this.congresoTagRepo.findOne({ where: { id: congresoTagId } });
    if (!congresoTag) throw new NotFoundException('Asignación de etiqueta a congreso no encontrada');
   ...
   */
  async validateRevisorForCongreso(revisorId: string, congresoId: string): Promise<{ isValid: boolean; message?: string }> {
    ...
    const congresoTagNames = congresoTags.map(ct => ct.tag.nombre.toLowerCase());
    ...
    const revisorTagNames = revisorTags.map(rt => rt.tag.nombre.toLowerCase());
    ...
    const hasMatch = congresoTagNames.some(tag => revisorTagNames.includes(tag));
   ...
   */
  async validateEditorForCongreso(editorId: string, congresoId: string): Promise<{ isValid: boolean; message?: string }> {
    // Obtener las etiquetas del congreso
    const congresoTags = await this.congresoTagRepo.find({
      where: { congreso_id: congresoId },
      relations: ['tag']
    ...
    // Verificar si hay al menos una coincidencia
    const hasMatch = congresoTagNames.some(tag => editorTagNames.includes(tag));
