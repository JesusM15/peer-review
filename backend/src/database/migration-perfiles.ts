...
async function runMigration() {
  ...
  console.log(
    `Buscando usuarios sin perfil (Total encontrados: ${users.length})`,
  );
  ...
  const defaultData = {
    [Rol.AUTOR]: {
      ...
      especialidades: ['Desarrollo Web', 'UX/UI'],
    },
    [Rol.REVISOR]: {
      carrera: 'Ciencias de la Computación',
      especialidades: ['Ciberseguridad', 'IA', 'Cloud'],
    },
    [Rol.EDITOR]: {
      carrera: 'Gestión Editorial',
      especialidades: ['Revision de Pares', 'Calidad'],
    },
    [Rol.ADMIN]: {
      carrera: 'Arquitectura de Sistemas',
      especialidades: ['DevOps', 'Seguridad'],
    },
  };

  for (const user of users) {
    const data = defaultData[user.rol] || {
    ...
    if (!user.perfil) {
      console.log(
        `[+] Creando perfil para usuario: ${user.email} (${user.id})`,
      );
      ...
      console.log(
        `[~] Actualizando especialidades para usuario: ${user.email}`,
      );
