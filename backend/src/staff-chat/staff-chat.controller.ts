...
export class StaffChatController {
  constructor(
    private readonly staffService: StaffChatService,
    private readonly congresosService: CongresosService,
  ) {}
  ...
  async getHistory(
    @Request() req,
    @Param('congresoId') congresoId: string,
    ...
    const allowed = memberships.some(
      (m) =>
        m.congreso_id === congresoId &&
        (m.rol === Rol.EDITOR || m.rol === Rol.ADMIN || m.rol === Rol.REVISOR),
    );
    if (!allowed)
      throw new BadRequestException('No autorizado en este congreso');
